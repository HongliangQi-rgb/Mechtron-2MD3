              ...
              bool animate, bool partially_observable = false)
{
  ...
  for (int i = 0; i < num_episode; i++)
  {
    ...
    // 如果是部分可观测模式，初始化 memory 保存初始时刻的 X 位置
    ...
    while (!env.terminal())
    {
      ...
      if (partially_observable)
      {
        // 部分可观测模式下，环境不提供速度信息（即 b 参数为 0）
        // 这里使用 auto regression 的思想，通过当前 X 值与上一次 X 值（存储在 memory 中）的差值来估计速度
        // 假设时间步长 TAU 为 0.02，则估计速度 estimated_v = (current_x - memory) / TAU
        double estimated_v = (current_x - memory) / 0.02;

        // 使用当前的 X 位置和估计的速度作为输入，调用表达式树获得控制动作
        action = t.evaluateExpression(current_x, estimated_v);

        // sidestep 更新：将当前的 X 位置保存到 memory 供下次使用
        memory = current_x;
      }
      else
      {
        // 全可观测模式下，直接使用环境提供的速度值
        action = t.evaluateExpression(current_x, env.getCartXVel());
      }

      // 根据动作更新环境，并累加本 episode 的分数
      episode_score += env.update(action, animate);
      episode_steps++;
    }

    // 统计所有 episode 的平均分数和步数
    mean_score += episode_score;
    mean_steps += episode_steps;
  }
