...
int arity(string op) {
  if (op == "abs")
    return 1;
  else if(op == "read"){ //read only read the memory, with no branches
    return 0;
  }
  else if (op == "write"){// write have only one branch
    return 1;
  }
  else
    return 2;
...
class LinkedBinaryTree {
 ...
 public:
  ...
  double getMemory() const{ //brief Computes the average value of all entries in the 'memory' container.
    double sum = 0;
    for (double v : memory)
      sum += v;
    return sum / memory.size(); 
  }
  void setMemory(double m){ //set new memory in the memory vector
    for (int i = 0; i < memory.size(); i++)
    {
      memory[i] = m;
    }
  }
  void updateMemory(double x) // Shifts the contents of 'memory' left by one position and appends a new value at the end
  {
 ...
 private:
  ...
  vector<double> memory; // use memory vector to achieve read and write
};
...
void LinkedBinaryTree::printExpression(Node* v)
{
  ...
  if (opArity == 0) { // if the node is read
    std::cout << v->elt;
  }
  else if (opArity == 1) { // for abs and write
    std::cout << v->elt << "(";
    if (v->left) {
...
double LinkedBinaryTree::evaluateExpression(const Position& p, double a,
  ...
  if (!p.isExternal()) {
    ...
    if (p.v->elt == "a")
      ...
      return b;
    else if (p.v->elt == "read"){ // add read and write for this function and use them 
...
void LinkedBinaryTree::addSubtreeMutator(mt19937 &rng, const int maxDepth, bool PARTIALLY_OBSERVABLE)// add a bool parameter to decide whether add write and read in the tree node
{
  if (_root == nullptr)
  {
    addRoot();
  ...
  int d = target -> depth(); 
  int depthAllowance = maxDepth - d; // add a depthAllowance to restrict the depth of tree
  if (depthAllowance < 0){
    depthAllowance = 0;
  }
  // generate a random subtree
  randomExpressionTree(target, depthAllowance, rng, PARTIALLY_OBSERVABLE);// use this function in the range of maxdepth
}
...
void LinkedBinaryTree::randomExpressionTree(Node *p, const int &maxDepth, mt19937 &rng , bool PARTIALLY_OBSERVABLE)//PARTIALLY_OBSERVABLE Whether we are in the partially observable setup or not.
{
  ...
  // if not in the leaf, then create an internal node, which is the operator, and then do the recursion
    ...
    if (opArity == 1) // for write and abs
    {
        p->left = new Node;
        p->left->par = p;
    ...
    else if (opArity == 0){// add read node in the tree
        p -> left = nullptr;
        p-> right = nullptr;
    }
  }
...
LinkedBinaryTree createRandExpressionTree(int max_depth, mt19937 &rng, bool PARTIALLY_OBSERVABLE)// bool number can decide whether use partially_observale function
{
...
void evaluate(mt19937& rng, LinkedBinaryTree& t, const int& num_episode,
  ...
  for (int i = 0; i < num_episode; i++) {
    ...
    if(PARTIALLY_OBSERVABLE){ // for part 4 if it is partially_observable, set all memory to be zero
      t.setMemory(0.0);
    }
...
public:
  ...
  {
    // If scores are nearly equal, compare by size (larger size -> "less than").
    if (fabs(A.getScore() - B.getScore()) < 0.01)
    {
      return A.size() > B.size();
    }
    // Otherwise, strictly compare by score (lower score -> "less than").
    else
    {
...
bool crossover(LinkedBinaryTree &treeA , LinkedBinaryTree &treeB, mt19937 &rng,int maxDepth){
    // Collect candidate internal nodes from treeA (non-root nodes).
  ...
  // Collect candidate internal nodes from treeB (non-root nodes)
  ...
  // If either tree has no internal candidates, we can't crossover.
  if (candidates1.empty() || candidates2.empty()){
    return false;
  }
// Randomly pick one nodeA, one nodeB.
...
// Swap in parent's child pointer
...
// Update nodeA->par, nodeB->par
  nodeA->par = parentB;
  nodeB->par = parentA;
// Check if either tree now exceeds maxDepth
...
int main() {
  ...
  const bool USECROSSOVER = false; // this is a switch, if want use crossover function, set it to true, if not, set it false
  ...
  for (int g = 1; g <= MAX_GENERATIONS; g++) {
    ...
    if (!USECROSSOVER){
        ...
        //  Mutation + crossover approach
        while (trees.size() < NUM_TREE) {

            double p = randDouble(rng);
            if (p < 0.5) { // 50% mutation, 50% to crossover mutation. 
                ...
                // Add resulting children back to the population
