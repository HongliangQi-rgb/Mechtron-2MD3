              ...
              bool animate, bool partially_observable = false)
{
  ...
  // evaluate the tree over num_episode episodes
  for (int i = 0; i < num_episode; i++)
  {
    ...
    // if in partially observable mode, store the previous X position
    double memory = env.getCartXPos();

    // run the episode until the cart reaches the terminal state
    while (!env.terminal())
    {
      int action;
      double current_x = env.getCartXPos();

      if (partially_observable)
      {
        // in partially observable mode, use the memory to estimate the speed
        // use auto-regression to update the memory
        // compute the speed as the difference between the current and previous X positions
        double estimated_v = (current_x - memory) / 0.02;

        // use the estimated speed to evaluate the action
        action = t.evaluateExpression(current_x, estimated_v);

        // update the memory to the current x
        memory = current_x;
      }
      else
      {
        // else then get the velocity
        action = t.evaluateExpression(current_x, env.getCartXVel());
      }
    ...
    // update the mean score and steps
