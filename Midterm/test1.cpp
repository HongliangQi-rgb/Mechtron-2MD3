...
public:
  LinkedBinaryTree() : _root(NULL), score(0), steps(0), generation(0), memoryEnabled(false) {}
  ...
  void setMemoryEnabled(bool flag) { memoryEnabled = flag; }
  bool isMemoryEnabled() const { return memoryEnabled; }
...
void LinkedBinaryTree::addSubtreeMutator(mt19937 &rng, const int maxDepth)
{
  ...
  randomExpressionTree(target, depthAllowance, rng);
}
